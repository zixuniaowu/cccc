...
def weave_system_prompt(home: Path, peer: str) -> str:
    ...
    lines.append("MANDATORY — Always append a <META> block to every message (place it at the end).")
    lines.append("Prefer the first item as ask/counter (with a next step or a ≤10‑min micro‑experiment); a second item may be mood/reflect.")
    lines.append("Example: to:peerB | kind:ask | msg: Two valid interpretations → write one acceptance example each, then converge | refs:[docs/weekly/…#L40-45]")
    lines.append("")
    ...
    lines.append("")
    # Always-on META channel (high-level, does not change code state)
    lines.append("Always-on <META> (high-level meta channel):")
    lines.append("• Append a <META> block to every message (always present; place it at the end).")
    ...
    lines.append("• Format: 1–2 items (soft cap), at least 1; one item per line: ")
    lines.append("  to:<peerA|peerB|system|user> | kind:<reflect|risk|ask|counter|mood> | msg:<actionable, specific> | refs:[…](opt)")
    lines.append("• Rules: ask/counter must include a next step or a ≤10‑min micro‑experiment; mood is welcome but keep it informative.")
    lines.append("• Routing: System→<FROM_SYSTEM>, PeerA→<FROM_PeerA>, PeerB→<FROM_PeerB>. <META> sits as a sibling block after the outer wrapper.")
    lines.append("")
    ...
    lines.append("• Always end with a <META> block (1–2 items; first ask/counter preferred).")
