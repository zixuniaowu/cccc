...
def weave_system_prompt(home: Path, peer: str) -> str:
    ...
    lines.append("MANDATORY — Always append an <INSIGHT> block to every message (place it at the end).")
    ...
    lines.append("• Append an <INSIGHT> block to every message (always present; place it at the end).")
    ...
    lines.append("• Routing: System→<FROM_SYSTEM>, PeerA→<FROM_PeerA>, PeerB→<FROM_PeerB>. <INSIGHT> sits as a sibling block after the outer wrapper.")
    lines.append("")
