...
def weave_system_prompt(home: Path, peer: str) -> str:
    ...
    lines.append("")
    lines.append("File Exchange (unified paths):")
    lines.append("• Inbound (from IM uploads): Orchestrator saves user-uploaded files to .cccc/work/upload/inbound/YYYYMMDD/MID__name and writes a sibling .meta.json (platform/chat-or-channel/mime/bytes/sha256/caption/mid/ts).")
    lines.append("• Outbound (to user): Drop files into .cccc/work/upload/outbound/{peer}/, optionally add <name>.caption.txt for message text; Telegram may use <name>.sendas (photo|document) to override auto-detection. On success a <name>.sent.json ACK is written.")
    lines.append("• Do not rely on platform-specific folders in paths; use the meta sidecar when platform/chat need to be known. Use the provided paths when citing evidence in messages or diffs.")
    lines.append("")
    lines.append("Rules:")
    ...
    lines.append("")
    # File exchange markers (inbound hints)
    lines.append("Inbound (from IM uploads): you will see <FROM_USER> sections containing file paths and SHA256/MIME/Size lines; use those paths for processing; cite them when relevant.")
    lines.append("")
