...
class TestActorPrivateEnv(unittest.TestCase):
    def test_codex_private_env_update_creates_project_config(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as td:
                ...
                attach, _ = handle_request(
                    DaemonRequest.model_validate(
                        {"op": "attach", "args": {"group_id": group_id, "path": str(repo), "by": "user"}}
                    )
                )
                ...
                self.assertEqual((upd.result or {}).get("codex_project_config_sync"), "updated")

                config_path = repo / ".codex" / "config.toml"
                self.assertTrue(config_path.exists())
                text = config_path.read_text(encoding="utf-8")
                self.assertIn('model_provider = "repo"', text)
                self.assertIn('[model_providers.repo]', text)
                self.assertIn('name = "repo"', text)
    ...
    def test_codex_private_env_update_refreshes_managed_project_config(self) -> None:
        from cccc.contracts.v1 import DaemonRequest
        from cccc.daemon.server import handle_request
        ...
        try:
            with tempfile.TemporaryDirectory() as td:
                ...
                (codex_dir / "config.toml").write_text(
                    "\n".join(
                        [
                            "# Managed by CCCC for per-project Codex runtime config.",
                            'model = "gpt-5.4"',
                            'model_provider = "repo"',
                            "",
                            "[model_providers.repo]",
                            'name = "repo"',
                            'base_url = "https://api.old.example.com/v1"',
                            'wire_api = "responses"',
                    ...
                    DaemonRequest.model_validate(
                        {
                            ...
                            "args": {
                                ...
                                "set": {
                                    "OPENAI_API_KEY": "supersecret",
                                    "OPENAI_BASE_URL": "https://api.new.example.com/v1",
                                },
                            },
                        }
                    )
                ...
                self.assertIn('base_url = "https://api.new.example.com/v1"', text)
                self.assertNotIn('base_url = "https://api.old.example.com/v1"', text)
        finally:
            if old_home is None:
                os.environ.pop("CCCC_HOME", None)
            else:
