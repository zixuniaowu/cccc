...
> ステータス：**0.4.0rc3**（Release Candidate）。UX と契約（contracts）を硬化中のため、破壊的変更の可能性があります。
...
RC tag（例：`v0.4.0-rc3`）は **TestPyPI** に公開されます。TestPyPI を主インデックスに、PyPI を依存解決の回退として使います：
