...
def handle_request(req: Dict[str, Any]) -> Tuple[Dict[str, Any], bool]:
    op = str(req.get("op") or "").strip()
    args = req.get("args") if isinstance(req.get("args"), dict) else {}

    if op == "ping":
        return {"ok": True, "version": __version__, "pid": os.getpid(), "ts": utc_now_iso()}, False

    if op == "shutdown":
        return {"ok": True, "message": "shutting down"}, True
    ...
    if op == "group_use":
        ...
        if not group_id:
            return {"ok": False, "error": "missing group_id"}, False
        group = load_group(group_id)
        if group is None:
            return {"ok": False, "error": f"group not found: {group_id}"}, False
        path = Path(str(args.get("path") or "."))
        scope = detect_scope(path)
        reg = load_registry()
        try:
            ...
            return {"ok": False, "error": str(e), "hint": "attach scope first (cccc attach <path> --group <id>)"}, False
        ev = append_event(
            group.ledger_path,
    ...
    if op == "groups":
        ...
        return {"ok": True, "groups": groups}, False
    ...
    if op == "send":
        ...
        if path:
            ...
            if not attached:
                return {"ok": False, "error": f"scope not attached: {scope_key}", "hint": "cccc attach <path> --group <id>"}, False
        else:
            scope_key = str(group.doc.get("active_scope_key") or "").strip()
        if not scope_key:
            return {"ok": False, "error": "missing scope_key (no active scope?)"}, False
        ev = append_event(
            group.ledger_path,
            kind="chat.message",
            group_id=group.group_id,
            scope_key=scope_key,
            by=by,
            data=ChatMessageData(text=text, format="plain").model_dump(),
        )
        return {"ok": True, "event": ev}, False
