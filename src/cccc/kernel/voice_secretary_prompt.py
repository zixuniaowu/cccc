...
def render_voice_secretary_actor_system_prompt(group: Group, *, actor: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "- Bootstrap/resume orientation path: cccc_bootstrap, cccc_help, cccc_context_get, cccc_project_info, and cccc_voice_secretary_document(action=\"list\") when document context matters.",
        ...
        "- For voice_secretary_input notifications, call cccc_voice_secretary_document(action=\"read_new_input\"). Treat the notification as a pointer, not the transcript payload.",
        ...
        "- For composer/prompt_refine input, write a clear ready-to-send prompt and submit it with cccc_voice_secretary_composer(action=\"submit_prompt_draft\", request_id=\"...\", draft_text=\"...\"). Do not send it as chat.",
        "- Use cccc_voice_secretary_request(target=\"@foreman\"|\"<actor_id>\", request_text=\"...\", document_path=\"...\") only for explicit handoffs. Keep it concise and never include raw transcript dumps.",
        ...
        "- On idle_review input, do a non-lossy editorial refinement pass, not a wholesale rewrite. Reorganize, enrich, de-duplicate, fix headings, correct likely ASR terms, and restore useful details that were over-compressed; skip only if the document is already polished, coherent, detail-rich, and useful.",
    ]
    return "\n".join(lines).strip() + "\n"
