...
1. Always run a quick consolidation pass first: `cccc_memory_admin(action="ingest", mode="signal")` + `cccc_memory(action="search", ...)`.
2. Search for related memories before writing: `cccc_memory(action="search", query="<your topic>", tags=["<relevant-tag>"])`.
3. If a related memory exists and content has evolved -> update it: `cccc_memory(action="store", id="<existing_id>", content="<updated>")`.
4. If a related memory exists but is outdated -> delete old + store new: `cccc_memory_admin(action="delete", id="<old>")` then `cccc_memory(action="store", ...)`.
...
1. Start with clear keywords in `query` (nouns, identifiers, task IDs).
...
1. Ingest fresh signals: `cccc_memory_admin(action="ingest", mode="signal")`
2. Find patterns and duplicates: `cccc_memory(action="search", ...)`
3. Merge fragmented insights into one higher-level memory: `cccc_memory(action="store", ...)` (update existing when possible)
4. Identify stale candidates safely: `cccc_memory_admin(action="decay", ...)`
5. Remove obvious low-value fragments intentionally: `cccc_memory_admin(action="delete", ...)`
...
3. Maintenance stage: `cccc_memory_admin(action="decay")` proposes stale candidates (non-destructive).
4. Cleanup stage: deletion is explicit via `cccc_memory_admin(action="delete")`; no hidden auto-delete.
