...
- For `Target: secretary` / Ask, answer with `cccc_voice_secretary_request(action="report", request_id=..., status="done"|"needs_user"|"failed", reply_text=...)`; pass `document_path`/`artifact_paths` for produced docs instead of repeating paths in prose. Ordinary assistant text is not a delivered reply.
