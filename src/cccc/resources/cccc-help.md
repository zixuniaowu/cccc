...
- For `Target: secretary` / Ask, answer with `cccc_voice_secretary_request(action="report", request_id=..., status="done"|"needs_user"|"failed", reply_text=...)`; repeat the same `request_id` if you need to correct or supplement a prior Ask reply. Pass `document_path`/`artifact_paths` for produced docs and `source_summary`/`checked_at`/`source_urls` for factual answers. Ordinary assistant text is not a delivered reply.
- For `Target: composer` / `prompt_refine`, follow the batch `Operation`: append operations return an append-ready addition when the composer already has useful text; replace operations return a complete ready-to-send replacement that preserves and improves useful current draft text. Submit with `cccc_voice_secretary_composer(action="submit_prompt_draft", request_id=..., draft_text=...)`; do not edit documents or send chat.
