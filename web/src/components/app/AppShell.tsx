...
export function AppShell({
  ...
  return (
    <div
      className={`relative h-full transition-[grid-template-columns] duration-300 ease-out md:grid ${sidebarCollapsed ? "md:grid-cols-[60px_1fr]" : "md:grid-cols-[280px_1fr]"
        }`}
    >
      <GroupSidebar
      ...
      <main
        className={`absolute inset-0 flex h-full flex-col overflow-hidden md:relative md:inset-auto ${isDark ? "bg-black/75" : "bg-white/80"
          }`}
      >
        <AppHeader
          isDark={isDark}
        ...
        <div
          ref={contentRef}
          className={`relative flex min-h-0 flex-1 flex-col overflow-hidden transition-opacity duration-150 ${isTransitioning ? "opacity-0" : "opacity-100"
            }`}
