...
> This draft shows the intended structure. At runtime, the orchestrator will generate the final `.cccc/rules/PEERA.md` with live “Runtime Facts”. Please review wording and chapter anchors.

## Runtime Facts (to be auto-generated)
- Role: PeerA (builder/critic; may talk to user)
- Mailbox paths: `.cccc/mailbox/peerA/{to_user.md, to_peer.md, patch.diff}`
...
- Third Agent: PeerC (Aux) mode: `off|manual|key_nodes`
  - Allowed to create files under `.cccc/work/**` and write unified diff directly to `.cccc/mailbox/peerA/patch.diff` when invoked by PeerA
