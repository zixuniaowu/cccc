...
```bash
# Basic reset: clears state/mailbox/logs/work and deletes context files
cccc reset

# Archive mode: moves context/ to timestamped archive before clearing
cccc reset --archive
```
...
- Resetting context and task files to begin fresh planning
